...
//
exports.clone = function (obj, depth = 5) {
  ...
  return clone(obj, depth);
};

function clone(obj, depth = 5) {
  //
  ...
  for (var i in obj) {
    if (obj.hasOwnProperty(i)) {
      if (obj[i] instanceof Error) return obj[i].stack;
      ...
      if (Array.isArray(obj[i])) {
        ...
        copy[i] = (obj[i] instanceof Object && depth) ? exports.clone(obj[i], depth - 1) : obj[i];
      }
      else if (typeof obj[i] === 'function') {
        copy[i] = obj[i];
      }
